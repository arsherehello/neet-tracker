...
public class MainActivity extends Activity {
    ...
    private TextView streakNum, countdownNum, todayTotalLine, smileMeter;
    private SubjectPieChart subjectPie;
    ...
    private LinearLayout buildHeader() {
        ...
        LinearLayout.LayoutParams divLp = new LinearLayout.LayoutParams(dp(2), dp(44));
        divLp.setMargins(dp(4), 0, dp(14), 0);
        divider.setLayoutParams(divLp);
        rightCol.addView(divider);

        subjectPie = new SubjectPieChart(this, null);
        LinearLayout.LayoutParams pieLp = new LinearLayout.LayoutParams(dp(46), dp(46));
        pieLp.setMargins(0, 0, dp(16), 0);
        subjectPie.setLayoutParams(pieLp);
        rightCol.addView(subjectPie);
        ...
        smileMeter = new TextView(this);
        smileMeter.setTextSize(18);
        smileMeter.setPadding(0, dp(4), 0, 0);
        streakBlock.addView(smileMeter);
    ...
    private void refreshHeader() {
        ...
        smileMeter.setText(moodEmoji(today));
        ...
        int[] doneCounts = new int[Syllabus.SUBJECT_KEYS.length];
        for (int i = 0; i < Syllabus.SUBJECT_KEYS.length; i++) {
            String key = Syllabus.SUBJECT_KEYS[i];
            doneCounts[i] = store.subjectCompletedCount(key);
            totalDone += doneCounts[i];
            totalAll += store.subjectTotalSubtasks(key);
            colors.put(key, Color.parseColor(Syllabus.SUBJECTS.get(key).color));
        }
        for (int i = 0; i < Syllabus.SUBJECT_KEYS.length; i++) {
            String key = Syllabus.SUBJECT_KEYS[i];
            shares.put(key, totalDone == 0 ? 0.0 : (double) doneCounts[i] / totalDone);
        }
        int overallPct = totalAll == 0 ? 0 : (int) Math.round(100.0 * totalDone / totalAll);
        subjectPie.setData(shares, colors, overallPct + "%");

        updateWidget();
    }

    private String moodEmoji(double hoursToday) {
        if (hoursToday <= 0) return "\uD83D\uDE34";       // sleeping
        if (hoursToday < 2) return "\uD83D\uDE10";        // neutral
        if (hoursToday < 4) return "\uD83D\uDE42";        // slight smile
        if (hoursToday < 6) return "\uD83D\uDE03";        // smile
        if (hoursToday < 8) return "\uD83D\uDE04";        // big smile
        return "\uD83E\uDD29";                            // star-struck
    }
    ...
    private LinearLayout buildTestsTab() {
        ...
        refresh[0] = new Runnable() {
            public void run() {
                ...
                // sort a copy by date for the trend chart
        ...
        addBtn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    store.addTest(subject, date, Double.parseDouble(scoreStr), 720.0);
                    scoreInput.setText("");
                    refresh[0].run();
