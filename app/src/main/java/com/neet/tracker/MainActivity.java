...
public class MainActivity extends Activity {
    ...
    private LinearLayout buildHeader() {
        ...
        // ---- rings row: per-subject completion, its own full-width line ----
        LinearLayout statsRow = new LinearLayout(this);
        statsRow.setOrientation(LinearLayout.VERTICAL);
        ...
        // countdown + streak get their own full-width row below the rings, instead of
        // squeezed beside them — cramming both rows into one horizontal line was wider
        // than the screen on many phones, pushing MIN/SEC/streak off the right edge.
        LinearLayout scCol = new LinearLayout(this);
        scCol.setOrientation(LinearLayout.HORIZONTAL);
        scCol.setGravity(Gravity.CENTER_VERTICAL);
        LinearLayout.LayoutParams scColLp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        scColLp.topMargin = dp(16);
        ...
        // spacer pushes the streak block to the right edge of the row
        View scSpacer = new View(this);
        scSpacer.setLayoutParams(new LinearLayout.LayoutParams(0, 0, 1f));
        scCol.addView(scSpacer);
        ...
        streakLabel.setSingleLine(true);
        ...
        moodTrack = new FrameLayout(this);
        moodTrack.setBackground(rounded(BORDER, 5));
        // The emoji (16sp) is taller than the 10dp bar, and FrameLayout clips children
        // to its own bounds by default — without this, almost the whole face was cut off.
        moodTrack.setClipChildren(false);
        moodTrack.setClipToPadding(false);
    ...
    private LinearLayout countdownUnit(TextView numView, String label) {
        ...
        // Wide enough for a 3-digit day count ("287") at 18sp bold without wrapping.
        col.setMinimumWidth(dp(34));
    ...
    private void refreshHeader() {
        ...
        // moodTrack is match_parent width, so use its real measured width, not a guess.
        // Before the first layout pass getWidth() is still 0, so fall back to the old
        // constant just for that one frame, then re-run once layout has happened.
        int trackW = moodTrack.getWidth();
        if (trackW <= 0) {
            trackW = dp(MOOD_BAR_WIDTH_DP);
            moodTrack.post(new Runnable() {
                public void run() { refreshHeader(); }
            });
        }
